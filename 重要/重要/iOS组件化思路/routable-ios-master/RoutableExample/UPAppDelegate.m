...
/*
 ...
 注册viewcontroller   :以VC形式注册，由于强制要求固定名称和参数的构造函数，以Runtime Call的方案，URLRoute不依赖各个模块（见源码）无需import对应类
 
 
 注册callback   :   以Block形式注册，由于执行代码是外部block传入的，外部要调用啥类还是需要import，因此在注册block的时候必然会产生模块依赖
